...
def prepare_target_images(img, target_id):

    if img.shape[0] > img.shape[1]:
        img = imutils.resize(img, height=470)
    else:
        img = imutils.resize(img, width=470)
    ...
    cv2.putText(text_area, f'Target ID : {target_id}', (200, 20), cv2.FONT_HERSHEY_PLAIN, 1, (0,0,0), 2)

    img = np.vstack([img, text_area])
